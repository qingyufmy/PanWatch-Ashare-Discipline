...
def _resolve_reduce_quantity(signal: StrategySignalRun, position_qty: int, price: float, nav: float) -> int | None:
    """Resolve an explicit partial reduction; never turn REDUCE into EXIT."""
    payload = signal.payload if isinstance(signal.payload, dict) else {}
    try:
        if payload.get("reduce_qty") is not None:
            close_qty = int(payload["reduce_qty"])
        elif payload.get("target_qty") is not None:
            close_qty = position_qty - int(payload["target_qty"])
        elif payload.get("target_weight") is not None:
            target_weight = float(payload["target_weight"])
            if not (0 < target_weight < 1) or price <= 0 or nav <= 0:
                return None
            target_qty = int((nav * target_weight / price) // FIXED_QUANTITY) * FIXED_QUANTITY
            close_qty = position_qty - target_qty
        else:
            return None
    except (TypeError, ValueError, OverflowError):
        return None
    return close_qty if 0 < close_qty < position_qty else None
...
class PaperTradingEngine:
    ...
    def _close_position(
        ...
        """记录全部或部分卖出；REDUCE 必须保留正数剩余仓位。"""
        now = _utc_now()
        quantity = pos.quantity if close_quantity is None else close_quantity
        if quantity <= 0 or quantity > pos.quantity:
            raise ValueError("invalid_close_quantity")
        if exit_reason == "signal_reduce" and quantity >= pos.quantity:
            raise ValueError("reduce_cannot_exit")
        # 含交易成本的净盈亏:卖出净回收 − 建仓含费投入(与建仓口径一致,资金守恒)
        buy_cost = -COST_MODEL.fill("buy", pos.entry_price, quantity).cash_delta
        sell_fill = COST_MODEL.fill("sell", exit_price, quantity)
        ...
        trade = PaperTradingTrade(
            ...
            closed_at=now,
            meta={"trigger_signal_run_id": trigger_signal_run_id} if trigger_signal_run_id else {},
        )
        db.add(trade)
        ...
        if remaining == 0:
            ...
            pos.unrealized_pnl = round(
                COST_MODEL.fill("sell", exit_price, remaining).cash_delta
                + COST_MODEL.fill("buy", pos.entry_price, remaining).cash_delta,
                4,
            )
        ...
        for pos in positions:
            ...
            if pos.signal_run_id:
                ...
                if latest and latest.action == "sell":
                    trade = self._close_position(db, account, pos, current_price, "signal_reversal", trigger_signal_run_id=latest.id)
                    exit_events.append((pos, trade))
                    closed += 1
                    continue
                if latest and latest.action == "reduce":
                    with db.no_autoflush:
                        prior = (
                            db.query(PaperTradingTrade)
                            .filter_by(stock_symbol=pos.stock_symbol, stock_market=pos.stock_market, exit_reason="signal_reduce")
                            .all()
                        )
                    if any((trade.meta or {}).get("trigger_signal_run_id") == latest.id for trade in prior):
                        continue
                    nav = account.current_capital + sum(
                        (p.current_price or p.entry_price) * p.quantity for p in positions if p.status == "open"
                    )
                    reduce_qty = _resolve_reduce_quantity(latest, pos.quantity, current_price, nav)
                    if reduce_qty is None:
                        logger.warning("[模拟盘] REDUCE 缺少有效部分减仓数量/目标仓位，保持持仓: %s", pos.stock_symbol)
                        continue
                    trade = self._close_position(
                        db, account, pos, current_price, "signal_reduce",
                        close_quantity=reduce_qty, trigger_signal_run_id=latest.id,
                    )
                    exit_events.append((pos, trade))
                    continue
