...
def _m127_portfolio_truth_and_position_plans(conn: Connection) -> None:
    """P1 append-only portfolio evidence, plans and transition audit."""
    conn.execute(text("""
        CREATE TABLE IF NOT EXISTS portfolio_truth_snapshots (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            trade_date TEXT NOT NULL, phase TEXT NOT NULL, source TEXT NOT NULL,
            fetched_at DATETIME NOT NULL, source_asof DATETIME,
            freshness TEXT NOT NULL, truth_status TEXT NOT NULL,
            logical_hash TEXT NOT NULL, anomaly_flags JSON NOT NULL,
            account_details JSON NOT NULL, cash REAL, nav REAL,
            created_at DATETIME DEFAULT CURRENT_TIMESTAMP
        )
    """))
    conn.execute(text("""
        CREATE TABLE IF NOT EXISTS portfolio_truth_positions (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            snapshot_id INTEGER NOT NULL REFERENCES portfolio_truth_snapshots(id),
            market TEXT NOT NULL, symbol TEXT NOT NULL, name TEXT NOT NULL,
            total_qty INTEGER NOT NULL, sellable_qty INTEGER,
            today_locked_qty INTEGER, avg_cost REAL NOT NULL,
            market_value REAL, account_details JSON NOT NULL,
            CONSTRAINT ux_truth_symbol UNIQUE(snapshot_id, market, symbol)
        )
    """))
    conn.execute(text("""
        CREATE TABLE IF NOT EXISTS position_plans (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            market TEXT NOT NULL, symbol TEXT NOT NULL, version INTEGER NOT NULL,
            position_state TEXT NOT NULL, thesis_state TEXT NOT NULL,
            plan JSON NOT NULL, logical_hash TEXT NOT NULL,
            truth_snapshot_id INTEGER REFERENCES portfolio_truth_snapshots(id),
            created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
            CONSTRAINT ux_position_plan_version UNIQUE(market, symbol, version)
        )
    """))
    conn.execute(text("""
        CREATE TABLE IF NOT EXISTS position_state_events (
            ...
            from_thesis TEXT, to_thesis TEXT NOT NULL,
            decision TEXT NOT NULL, reason TEXT NOT NULL DEFAULT '',
            plan_version INTEGER, created_at DATETIME DEFAULT CURRENT_TIMESTAMP
        )
    """))
    _create_index_if_missing(conn, "ix_truth_trade_date_phase", "CREATE INDEX ix_truth_trade_date_phase ON portfolio_truth_snapshots(trade_date, phase, id)")
    _create_index_if_missing(conn, "ix_position_plan_current", "CREATE INDEX ix_position_plan_current ON position_plans(market, symbol, version)")
    _create_index_if_missing(conn, "ix_position_state_event_symbol", "CREATE INDEX ix_position_state_event_symbol ON position_state_events(market, symbol, id)")


def _m128_signal_execution_journal(conn: Connection) -> None:
    """P2 append-only signal, policy, execution and reconciliation records."""
    statements = (
        """CREATE TABLE IF NOT EXISTS evidence_snapshots (
            id INTEGER PRIMARY KEY AUTOINCREMENT, captured_at DATETIME NOT NULL,
            source TEXT NOT NULL, logical_hash TEXT NOT NULL, payload JSON NOT NULL,
            truth_snapshot_id INTEGER REFERENCES portfolio_truth_snapshots(id),
            created_at DATETIME DEFAULT CURRENT_TIMESTAMP)""",
        """CREATE TABLE IF NOT EXISTS signal_events (
            signal_id TEXT PRIMARY KEY, trace_id TEXT NOT NULL, trade_date TEXT NOT NULL,
            market TEXT NOT NULL, symbol TEXT NOT NULL, source TEXT NOT NULL,
            source_agent TEXT NOT NULL DEFAULT '',
            source_suggestion_id INTEGER REFERENCES stock_suggestions(id),
            plan_version INTEGER, truth_snapshot_id INTEGER REFERENCES portfolio_truth_snapshots(id),
            evidence_snapshot_id INTEGER NOT NULL REFERENCES evidence_snapshots(id),
            action TEXT NOT NULL, raw_action TEXT NOT NULL, current_weight REAL,
            target_weight REAL, delta_weight REAL, qty_hint INTEGER,
            generated_at DATETIME NOT NULL, valid_from DATETIME NOT NULL,
            expires_at DATETIME NOT NULL, dedupe_key TEXT NOT NULL UNIQUE,
            status TEXT NOT NULL, reason_codes JSON NOT NULL,
            prompt_id TEXT, prompt_version TEXT, model_role TEXT,
            requested_model TEXT, reported_model TEXT,
            created_at DATETIME DEFAULT CURRENT_TIMESTAMP)""",
        ...
        """CREATE TABLE IF NOT EXISTS signal_policy_decisions (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            signal_id TEXT NOT NULL REFERENCES signal_events(signal_id),
            rule_id TEXT NOT NULL, input_hash TEXT NOT NULL,
            decision TEXT NOT NULL, reason_codes JSON NOT NULL,
            created_at DATETIME DEFAULT CURRENT_TIMESTAMP)""",
        """CREATE TABLE IF NOT EXISTS execution_events (
            execution_id TEXT PRIMARY KEY,
            signal_id TEXT NOT NULL REFERENCES signal_events(signal_id),
            planned_action TEXT NOT NULL, planned_qty INTEGER, planned_weight REAL,
            actual_action TEXT NOT NULL, actual_qty INTEGER NOT NULL, actual_price REAL,
            execution_source TEXT NOT NULL, executed_at DATETIME,
            result TEXT NOT NULL, notes TEXT NOT NULL DEFAULT '',
            reconcile_status TEXT NOT NULL DEFAULT 'PENDING',
            reconcile_truth_snapshot_id INTEGER REFERENCES portfolio_truth_snapshots(id),
            created_at DATETIME DEFAULT CURRENT_TIMESTAMP)""",
        """CREATE TABLE IF NOT EXISTS discipline_events (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            signal_id TEXT NOT NULL REFERENCES signal_events(signal_id),
            event_type TEXT NOT NULL, details JSON NOT NULL,
            created_at DATETIME DEFAULT CURRENT_TIMESTAMP)""",
        """CREATE TABLE IF NOT EXISTS next_day_actions (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            signal_id TEXT NOT NULL REFERENCES signal_events(signal_id),
            market TEXT NOT NULL, symbol TEXT NOT NULL,
            due_trade_date TEXT NOT NULL, pending_qty INTEGER NOT NULL,
    ...
    indexes = (
        ("ix_signal_market_symbol_created", "CREATE INDEX ix_signal_market_symbol_created ON signal_events(market, symbol, generated_at)"),
        ("ix_signal_status_expires", "CREATE INDEX ix_signal_status_expires ON signal_events(status, expires_at)"),
        ("ix_signal_lifecycle_signal", "CREATE INDEX ix_signal_lifecycle_signal ON signal_lifecycle_events(signal_id, id)"),
        ("ix_signal_policy_signal", "CREATE INDEX ix_signal_policy_signal ON signal_policy_decisions(signal_id, id)"),
        ("ix_execution_signal", "CREATE INDEX ix_execution_signal ON execution_events(signal_id, execution_id)"),
        ("ix_discipline_signal", "CREATE INDEX ix_discipline_signal ON discipline_events(signal_id, id)"),
        ("ix_next_day_due_status", "CREATE INDEX ix_next_day_due_status ON next_day_actions(due_trade_date, status)"),
    )
    for name, statement in indexes:
        _create_index_if_missing(conn, name, statement)


def _m129_actionable_signal_envelope(conn: Connection) -> None:
    conn.execute(text("""
        CREATE TABLE IF NOT EXISTS actionable_signals (
            signal_id TEXT PRIMARY KEY REFERENCES signal_events(signal_id),
            approved_qty INTEGER, approved_weight REAL,
            policy_version TEXT NOT NULL, input_hash TEXT NOT NULL,
            approved_at DATETIME NOT NULL
        )
    """))


def _m130_model_profiles_and_runs(conn: Connection) -> None:
    conn.execute(text("""
        CREATE TABLE IF NOT EXISTS model_profiles (
            role TEXT PRIMARY KEY, ai_model_id INTEGER NOT NULL REFERENCES ai_models(id),
            thinking TEXT NOT NULL DEFAULT 'default', reasoning_effort TEXT,
            max_tokens INTEGER NOT NULL, timeout_seconds INTEGER NOT NULL,
            retries INTEGER NOT NULL DEFAULT 0, fallback_role TEXT,
    ...
    conn.execute(text("""
        CREATE TABLE IF NOT EXISTS model_runs (
            ...
            role TEXT NOT NULL, profile_role TEXT NOT NULL,
            requested_model TEXT NOT NULL, reported_model TEXT,
            prompt_id TEXT, prompt_version TEXT, prompt_hash TEXT,
            input_hash TEXT NOT NULL, input_tokens INTEGER, output_tokens INTEGER,
            cost_usd REAL, latency_ms INTEGER NOT NULL,
            fallback_used BOOLEAN NOT NULL DEFAULT 0,
            schema_valid BOOLEAN, status TEXT NOT NULL, error TEXT,
            started_at DATETIME NOT NULL, finished_at DATETIME NOT NULL
        )
    """))
    _create_index_if_missing(conn, "ix_model_runs_role_started", "CREATE INDEX ix_model_runs_role_started ON model_runs(role, started_at)")


def _m131_prompt_versions(conn: Connection) -> None:
    conn.execute(text("""
        CREATE TABLE IF NOT EXISTS prompt_versions (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            prompt_id TEXT NOT NULL, version TEXT NOT NULL,
            system_template TEXT NOT NULL, input_schema JSON NOT NULL,
            output_schema JSON NOT NULL, model_role TEXT NOT NULL,
            change_reason TEXT NOT NULL, parent_version TEXT,
            prompt_hash TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'CANDIDATE',
            created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
...
def _m132_model_run_output_evidence(conn: Connection) -> None:
    _add_column_if_missing(conn, "model_runs", "output_hash",
                           "ALTER TABLE model_runs ADD COLUMN output_hash TEXT")
    _add_column_if_missing(conn, "model_runs", "output_text",
                           "ALTER TABLE model_runs ADD COLUMN output_text TEXT")


def _m133_system_issues(conn: Connection) -> None:
    conn.execute(text("""
        CREATE TABLE IF NOT EXISTS system_issues (
            issue_id TEXT PRIMARY KEY, error_signature TEXT NOT NULL,
            ...
            counts_by_day JSON NOT NULL, contexts JSON NOT NULL, root_cause TEXT,
            fix_commit TEXT, regression_test TEXT, resolved_at DATETIME
        )
    """))
    _create_index_if_missing(conn, "ix_system_issues_signature_status",
                             "CREATE INDEX ix_system_issues_signature_status ON system_issues(error_signature, status)")


def _m134_portfolio_daily_workflow(conn: Connection) -> None:
    _add_column_if_missing(conn, "signal_events", "daily_plan_version",
                           "ALTER TABLE signal_events ADD COLUMN daily_plan_version INTEGER")
    conn.execute(text("""
        CREATE TABLE IF NOT EXISTS daily_portfolio_plans (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            trade_date TEXT NOT NULL, version INTEGER NOT NULL,
            status TEXT NOT NULL, truth_snapshot_id INTEGER REFERENCES portfolio_truth_snapshots(id),
            ...
            payload JSON NOT NULL, created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
            CONSTRAINT uq_daily_plan_date_version UNIQUE (trade_date, version)
        )
    """))
    conn.execute(text("""
        CREATE TABLE IF NOT EXISTS portfolio_workflow_runs (
            run_id TEXT PRIMARY KEY, trade_date TEXT NOT NULL,
            step TEXT NOT NULL, slot TEXT NOT NULL,
            status TEXT NOT NULL, input_hash TEXT, output_hash TEXT,
            payload JSON NOT NULL, started_at DATETIME NOT NULL,
            finished_at DATETIME,
            CONSTRAINT uq_portfolio_workflow_slot UNIQUE (trade_date, step, slot)
        )
    """))
...
MIGRATIONS: tuple[Migration, ...] = (
    ...
    Migration(127, "portfolio_truth_and_position_plans", _m127_portfolio_truth_and_position_plans),
    Migration(128, "signal_execution_journal", _m128_signal_execution_journal),
    Migration(129, "actionable_signal_envelope", _m129_actionable_signal_envelope),
    Migration(130, "model_profiles_and_runs", _m130_model_profiles_and_runs),
    Migration(131, "prompt_versions", _m131_prompt_versions),
