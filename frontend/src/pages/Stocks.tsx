...
interface PortfolioSummary {
  ...
  total: {
    ...
    available_funds: number
    total_assets: number
    declared_nav?: number | null
    declared_nav_status?: string | null
  }
  exchange_rates?: {
    HKD_CNY: number
    USD_CNY?: number
...
const mergePortfolioQuotes = (
  ...
  return {
    ...
    total: {
      ...
      declared_nav: portfolio.total.declared_nav,
      declared_nav_status: portfolio.total.declared_nav_status,
    },
  }
}
...
export default function StocksPage() {
  ...
  const positionRatio = useMemo(() => {
    ...
    const assets = portfolio.total.declared_nav || portfolio.total.total_assets || 0
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
          ...
          <div className="card p-4">
            <div className="flex items-center gap-2 text-muted-foreground mb-1">
              ...
              <span className="text-[12px]">{portfolio.total.declared_nav ? '用户声明总资产' : '录入资产估计'}</span>
            </div>
            <div className="text-[20px] font-bold text-foreground font-mono">
              {formatMoney(portfolio.total.declared_nav || portfolio.total.total_assets)}
            </div>
            {portfolio.total.declared_nav ? <div className="text-[11px] text-muted-foreground">未核对券商；不代表可用资金</div> : null}
          </div>
