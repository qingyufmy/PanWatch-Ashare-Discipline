...
from src.platform.persistence.models import Account, PortfolioTruthSnapshot, PriceAlertRule, Position, Stock
...
def get_portfolio_summary(
    ...
    Returns:
        ...
        total: 所有账户汇总
    """
    latest_truth = (db.query(PortfolioTruthSnapshot).filter(PortfolioTruthSnapshot.nav.isnot(None))
                    .order_by(PortfolioTruthSnapshot.id.desc()).first()) if account_id is None else None
    declared_nav = latest_truth.nav if latest_truth else None
    # 获取账户
    if account_id:
    ...
    if not accounts:
        return {
            ...
            "total": {
                ...
                "declared_nav_status": latest_truth.truth_status if latest_truth else None,
            }
        }
