...
def build_scheduler() -> AgentScheduler:
    ...
    from src.modules.portfolio.daily_workflow import register_jobs
...
async def lifespan(app):
    ...
    try:
        ...
        calendar_refresh_task = asyncio.create_task(refresh_trading_calendar())
    except Exception as e:
    ...
    async def recover_portfolio_workflow():
        if calendar_refresh_task:
            await calendar_refresh_task
        from src.modules.portfolio.daily_workflow import recover_missed

        await asyncio.to_thread(recover_missed)

    asyncio.create_task(recover_portfolio_workflow())
    try:
        settings = Settings()
