...
def _format_exit_message(pos: dict, trade: dict) -> tuple[str, str]:
    ...
    partial = trade.get("exit_reason") == "signal_reduce"
    title = f"【模拟盘{'减仓' if partial else '平仓'}】{name} {pnl_sign}{pnl:.2f}"

    stock_info = _stock_display(pos["stock_symbol"], pos["stock_market"], name)
    reason = EXIT_REASON_LABELS.get(trade["exit_reason"], trade["exit_reason"])
    quantity_line = f"卖出数量: {trade['quantity']} 股\n" if trade.get("quantity") is not None else ""
    body = (
        f"股票: {stock_info}\n"
        f"{'减仓' if partial else '平仓'}原因: {reason}\n"
        f"{quantity_line}"
