...
class IntradayMonitorAgent(BaseAgent):
    ...
    async def run(self, context: AgentContext) -> AnalysisResult:
        ...
        active = [
            stock for stock in context.watchlist
            if self.bypass_market_hours or is_market_trading(stock.market)
        ]
        if not active:
            return AnalysisResult(
                agent_name=self.name,
                title="【盘中监测】非交易时段",
                content="当前持仓所属市场均未开盘",
                raw_data={"skipped": True, "notified": False},
            )

        builder = SignalPackBuilder()
        packs = await builder.build_for_symbols(
            symbols=[(s.symbol, s.market, s.name) for s in active],
            include_news=True,
            news_hours=24,
        ...
        missing = [s.symbol for s in active if not packs.get(s.symbol) or not packs[s.symbol].quote]
        if missing:
        ...
        for stock in active:
            ...
            quote = pack.quote
            technical = pack.technical or {}
            positions = context.portfolio.get_positions_for_stock(stock.symbol)
            position_rows = []
            for pos in positions:
                pnl = (
                    (quote.current_price - pos.cost_price) / pos.cost_price * 100
                    if quote.current_price is not None and pos.cost_price > 0 else None
                )
            ...
            inputs.append({
                ...
                "technical": {k: technical.get(k) for k in (
                    "trend", "macd_status", "rsi6", "rsi_status", "kdj_status",
                    "boll_status", "volume_ratio", "atr_pct", "ma5", "ma10",
                    "ma20", "ma60", "support_s", "resistance_s", "kline_pattern",
                )},
        ...
        user_content = json.dumps({
            ...
            "total_available_funds": context.portfolio.total_available_funds,
            "thresholds": {
                "price_change_pct": self.price_alert_threshold,
                "volume_ratio": self.volume_alert_ratio,
    ...
    @staticmethod
    def _parse_batch_response(content: str, expected: set[str]) -> dict[str, dict]:
        ...
        try:
            ...
            raise ValueError("盘中批量模型输出不是有效 JSON") from exc
        items = parsed.get("items") if isinstance(parsed, dict) else None
        if not isinstance(items, list):
            raise ValueError("盘中批量模型输出缺少 items")
        by_symbol = {}
        for item in items:
            if not isinstance(item, dict):
                raise ValueError("盘中批量模型输出包含非对象项")
            symbol = item.get("symbol")
            action = item.get("action")
            if symbol not in expected or symbol in by_symbol or action not in _BATCH_ACTIONS:
                raise ValueError("盘中批量模型输出包含未知、重复股票或非法动作")
            by_symbol[symbol] = item
        if set(by_symbol) != expected:
            raise ValueError("盘中批量模型输出未覆盖全部持仓")
        return by_symbol
